...
**核心交互**：摄像头手势识别（MediaPipe GestureRecognizer）——检测到手后，**地球粒子实时溶解变形为你手部的形状**（任意手势均可复刻，含握拳/比耶/点赞/张掌等），手放下后地球流回原状。
...
| 手势指示球 | 顶部白色圆球（华为 Mate 80 隔空手势风格） | 显示当前手势 SVG 图标 + 置信度弧光，无手缩回 |

**当前已移除**（按用户要求）：
- ❌ 外层蓝色防护罩（`buildShell` / 菲涅尔球壳）→ 已删除
- ❌ 屏幕中央悬浮蓝圈（`#ring` / `updateRing`）→ 已删除
- ❌ 左下角摄像头小窗 → 已隐藏（`#camv` 屏幕外运行，仅用于手势识别取流）
...
### ⚠️ 重要：shader 兼容性红线（血的教训）

**这个项目的 VERT 着色器必须保持简单结构**。曾经尝试把掌心做成"多边形掌面"（6 个三角扇区 + if-else 链 + step 函数 + 复杂分支），结果**部分手机 GPU（如鸿蒙浏览器）编译失败 → 整个地球粒子全部消失**（陆地不见了、只剩雾球轮廓）。
...
- **模型**：Google MediaPipe GestureRecognizer（浏览器端最强方案，官方 2.7 万★）
...
- 前置摄像头是**镜像画面**：手往左移 → cx 减小 → 需 `gestYaw -= dx`（负号对齐直觉，之前符号反了）
- 增益从 3.2 降到 1.6（原来幅度过大）
- 加死区（<0.012 忽略）+ EMA 平滑（单帧抖动只贡献 30%）防飘
...
> 额外发现：world-atlas 的 land-10m 环缠绕方向与 d3 的球面约定不一致，实测 `geoContains` 会把大西洋、几内亚湾、南太平洋也判成陆地（恒为 `true`）。
...
陆地粒子是白色（HSL(0,0,0.92)），但着色器夜侧 tint 是蓝色（`0.26,0.34,0.55`），`白色 × 蓝色 = 深蓝` → 白色陆地会被染成跟海洋一样的颜色**看不见**。

**当前方案**：VERT 里检测白色粒子（亮度 > 0.8 且近似无色）→ `tint = mix(tint, vec3(1.0), isWhiteLand)` 强制纯白。这是简单乘法+step，**不影响 GPU 兼容性**。
...
- **后果**：只改 index.html 不升 SW 版本 → 静态资源用旧缓存，MediaPipe 二进制与新版代码不匹配 → **手势识别失败**。升级版本号（v7→v8）强制清旧缓存解决
- **用户强刷须知**：Ctrl+Shift+R 或清缓存才能看到新版；SW 升级需"访问两次"才完全生效（第一次装新 SW，第二次生效）
...
## 八、GitHub 仓库维护（给接手 AI）

- **账号**：`111qqq333www444`
- **仓库**：`homepage`（粒子地球，main 分支）——线上 <https://111qqq333www444.github.io/homepage/>
- 其他仓库：`farewell`（UI 图标资源库）、`zjt-labs`（原 zjt-studio）
- **推送方式**：GitHub API `PUT /repos/{owner}/{repo}/contents/{path}`（base64 content + sha + branch=main），先 GET 拿 sha 再 PUT
- **密钥**：见交接文档（**不要写进公开 README**，GitHub 会扫描吊销）

---

## 九、当前状态与待办（2026-09-21）
...
| 蓝色防护罩 / 悬浮圆圈 | ❌ 已删除（用户要求） |
| 摄像头小窗 | ❌ 已隐藏 |
| **手形像真手**（核心待办） | ⚠️ **仍不够像**：掌心是圆盘感、手指略粗/略细待调、拇指位置待优化 |
| 手势方向识别精度 | ⚠️ 基本可用，仍有偶发反向/幅度问题 |
| 预设模型切换（星云/烟花/土星/花朵） | ⏳ 未做 |
| 城市灯光分级亮度 | ⏳ 未做 |

### 待优化方向（给接手 AI 的参考）

1. **手形"像真手"是用户最核心诉求**，当前是"圆盘掌心 + 圆柱手指"方案。安全改进路线：
   - JS 层调骨骼段：手指半径、掌心盘动态系数（×1.8 → 可调）、拇指连接段方向
   - 掌心圆盘 → 椭圆盘（shader 一行乘法，兼容安全）
   - 手指加"指腹"段（每指节末端加粗再 taper）
   - 严禁重蹈覆辙：不要上复杂 shader 分支（多边形掌面曾导致地球消失）
